  ...
  <div>
    <v-card max-width="800" v-for="(post,x) in posts" :key="x" class="mx-auto" id="post">
      ...
      <div class="post-container">
        <v-img :src="`http://localhost:3000/files/${post.post_image}`" height="500"></v-img>
      ...
      <v-card-actions>
        ...
        <div class="text-center">
          <v-rating v-model="rating" background-color="yellow" color="yellow" x-large v-show="$route.path === '/dashboard'"></v-rating>
        </div>
        <!-- Comment Dialog -->
      </v-card-actions>
